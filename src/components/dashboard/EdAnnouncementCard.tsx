import { EdAnnouncement } from '@/lib/types';
import { getCourseColor } from '@/lib/courseColors';
import { SourceBadge } from '@/components/ui/SourceBadge';
import { Pin } from 'lucide-react';

type EdAnnouncementCardProps = {
  announcement: EdAnnouncement;
  url?: string | null;
};

export function EdAnnouncementCard({ announcement, url }: EdAnnouncementCardProps) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <SourceBadge source="ed" />
            {announcement.isPinned && (
              <Pin className="w-3.5 h-3.5 text-[#A3A3A3]" />
            )}
          </div>
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            {announcement.linkedAssignment && (
              <>
                <span className="text-[#525252]">&bull;</span>
                <span className="text-[#525252]">re: {announcement.linkedAssignment}</span>
              </>
            )}
          </div>
        </div>
